...
- **Japanese pages: year-less date lines + school-grade ages.** A JP listing
  ...
  `Session`) — see `tokyo_ballet_school`, `tokyo_city_ballet`. A company's
  short-term workshop can live on a **competition microsite** while its school
  site has no workshop page — scrape the dedicated workshop page there, but anchor
  on its structured 開催概要/受講料 blocks: such pages keep **stale prior-edition
  prose** (commented-out 中止 lines, past-year admin dates) that loose-text parsing
  would catch (see `tokyo_city_ballet`).
